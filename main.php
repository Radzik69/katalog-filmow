...
<body id="bodyMain">
  ...
  } else if ($_SESSION["rights"] == "worker") {
    echo "
    <script>
      sessionStorage.setItem('rights', 'worker')
    </script>";
  } else {
    echo "
      <script>
  ...
  <script>
    ...
    function buttonClickList() {
      ...
      if (buttonIfClicked <= 1) {
        ...
        if (window.sessionStorage.getItem("rights") == "worker") {
          const worker = document.createElement("button")
          worker.setAttribute("class", "accountButtons")
          worker.innerHTML = "Update DB"
          worker.setAttribute("onclick", "UpdateDBGoTo()")
          button.appendChild(worker)
        }
    ...
    function UpdateDBGoTo() {
      window.location = "updateDB.php";
    }
